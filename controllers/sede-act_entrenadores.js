...
import { validateSedeActEntrenadores, validateParcialSedeActEntrenadores } from "../Schemas/sede-act_entrenadores.js";
...
export class sedeActEntrenadorController {
    ...
    static async getAll(req, res) {
        try{
            ...
            const sedeActEntrenadores = await parametrosQueryGetAll(idSedeAct, idEntrenador);
            if(sedeActEntrenadores.length === 0){
                res.status(404).json({msg: "No se encontraron sedes-actividades asociadas a un entrenador"});
            } else {
    ...
    static async getById(req, res) {
        try{
            const sedeActEntrenador = await sedeActEntrenadorModel.findByPk(req.params.id, {
                include: [
                    {
            ...
            if(!sedeActEntrenador){
                res.status(404).json({msg: "Sede-actividad asociada a un entrenador no encontrada"});
            } else {
    ...
    static async create(req, res) {
        try{
            ...
            if(result.error){
                res.status(400).json({msg: "Error en el ingreso de datos", error: result.error.message});
            }else{
                await sedeActEntrenadorModel.create(result.data);
                res.status(201).json({msg: "Asociacion sede-actividad con entrenador creada"});
            }
        } catch (error) {
            if(error.message.includes("foreign key constraint fails")){
                res.status(400).json({ msg: "La sede-actividad o el entrenador ingresado no existen"});
            }else{
                res.status(500).json({
    ...
    static async update(req, res) {
        try{
            ...
            if(result.error){
                ...
                const sedeActEntrenador = await sedeActEntrenadorModel.findByPk(req.params.id);
                if(!sedeActEntrenador){
                    res.status(404).json({msg: "Asociacion sede-actividad con entrenador no encontrada"});
                }else{
                ...
                res.status(200).json({msg: "Asociacion sede-actividad con entrenador actualizada"});
                }
            }
        } catch (error) {
            if(error.message.includes("foreign key constraint fails")){
    ...
    static async delete(req, res) {
        try{
            ...
            if(!sedeActEntrenador){
                ...
                res.status(200).json({msg: "Sede-actividad asociada a un entrenador eliminada"});
            }
        } catch (error) {
            res.status(500).json({
...
//Listado entrenadores por sede-actividad
//Listado de sedes-actividad de un entrenador
//Todos juntos (get all simple)
